...
public class ImpossibleDifDirControll {
    ...
    public String differentPath(String subTxt, String compileRes, Model model, HttpServletRequest request) {
        ...
        if (subTxt != null && cookie != null){
            session.setAttribute(cookie.getName()+"TrunkId", subTxt);
            session.setAttribute(cookie.getName() + "ResId", compileRes);
        }

        if (cookie != null)
            model.addAttribute("htmlId", session.getAttribute(cookie.getName()));
